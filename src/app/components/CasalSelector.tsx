...
export function CasalSelector({
  ...
  const selectedMacho = selectedCasal
    ? aves.find(a => a.id === selectedCasal.mId)
    : null;
  const selectedFemea = selectedCasal
    ? aves.find(a => a.id === selectedCasal.fId)
    : null;
    ...
    return (
      <div className="flex items-center gap-2 flex-1">
        {ave.photo ? (
          <button
            type="button"
            onClick={(event) => abrirFoto(event, ave.photo!)}
            ...
            aria-label={`Ampliar foto de ${
              ave.name || ave.ring || 'ave'
            }`}
          >
            <img
              src={ave.photo}
              alt={ave.name || ave.ring}
              className="w-8 h-8 rounded-full object-cover border-2 border-white shadow-sm cursor-zoom-in hover:scale-105 transition-transform"
            />
          </button>
        ) : (
          <div
        ...
        <button
          ...
          className={`w-full bg-white border-2 rounded-xl p-3 text-left transition-all outline-none flex items-center justify-between ${
            selectedCasal
              ...
              : 'border-amber-400 hover:border-amber-500 shadow-md animate-pulse'
          }`}
        >
          {selectedCasal && selectedMacho && selectedFemea ? (
            ...
            <div className="flex items-center gap-2">
              ...
              <span className="text-sm text-amber-600 font-bold">
                {placeholder || 'Clique para adicionar casal'}
              </span>
            </div>
          )}

          <i
            className={`fas fa-chevron-down text-slate-400 text-xs ml-2 transition-transform ${
              isOpen ? 'rotate-180' : ''
            }`}
          ></i>
        </button>
        ...
        {isOpen && (
          <div className="absolute top-full left-0 right-0 mt-2 bg-white border-2 border-slate-200 rounded-xl shadow-xl z-50 max-h-80 overflow-y-auto">
            ...
            {casaisFiltrados.map((casal) => {
              const macho = aves.find(
                a => a.id === casal.mId
              );

              const femea = aves.find(
                a => a.id === casal.fId
              );
