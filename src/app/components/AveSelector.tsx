      ...
      <div className="flex items-center gap-3">
        {ave.photo ? (
          <button
            ...
            aria-label={`Ampliar foto de ${ave.name || ave.ring || 'ave'}`}
          >
            <img
              src={ave.photo}
              alt={ave.name || ave.ring}
              className="w-10 h-10 rounded-full object-cover border-2 border-white shadow-sm cursor-zoom-in hover:scale-105 transition-transform"
            />
          </button>
        ) : (
    ...
    <>
      <div ref={dropdownRef} className={`relative ${className}`}>
        ...
        >
          ...
          <i
            className={`fas fa-chevron-down text-slate-400 text-xs ml-2 transition-transform flex-shrink-0 ${
              isOpen ? 'rotate-180' : ''
            }`}
          ></i>
        </button>
          ...
          <div className="absolute top-full left-0 right-0 mt-2 bg-white border-2 border-slate-200 rounded-xl shadow-xl z-50 max-h-80 overflow-y-auto">
            {aves.length === 0 ? (
              <div className="p-6 text-center text-slate-400">
                ...
                <p className="text-xs">
                  Nenhum{tipo === 'macho' ? '' : 'a'}{' '}
                  {tipo === 'macho' ? 'macho' : 'fêmea'} ativ
                  {tipo === 'macho' ? 'o' : 'a'} cadastr
                  {tipo === 'macho' ? 'o' : 'a'}
                </p>
              </div>
            ) : (
